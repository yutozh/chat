            ...
            try:
                ...
                if msg[0] == 0:
                    ...
                    if ClientGUI.listFriend != alluser_dict:
                        ...
                        self.listFriendname = []
                        self.listFriendip = []
                        ClientGUI.myname = ClientGUI.listFriend[ClientGUI.localip]["name"]
                        if flag == 1:
                ...
                # 新消息
                    ...
                    msg_id = remote_data["id"]
                    msg_content = remote_data["msg"]
...
class chatGUI(object):
    def __init__(self, id, name, queue):
        ...
        self.peerid = id
        self.peerip = ClientGUI.listFriend[self.peerid]["ip"]  # ip
        self.running = True
    ...
    def toSend(self):
        ...
        if message != "":
            ...
            if self.peerip == "x":
                ...
                print "sended"
                ClientConn.send_socket_default.sendto(data, ("127.0.0.2", 8666))  # 离线留言发送至服务器
            else:
                data = json.dumps(data)
    ...
    def processIncoming(self):
        queue = self.queue
        self.peerip = ClientGUI.listFriend[self.peerid]["ip"]  # 不断刷新ip，监控是否上线
        while queue.qsize():
            try:
                msg = queue.get(0)
                if msg:
                    peername = ClientGUI.listFriend[self.peerip]["name"]
...
class ClientConn(object):
    ...
    def __init__(self, id, token, master):
        ...
        # 用来发送的socket绑定
        ClientConn.send_socket.bind((self.local_ip, 6777))
